...
enum class Level1Navigation(val screenIdentifier: ScreenIdentifier, val rememberVerticalStack: Boolean = false) {
    AllArticles( ScreenIdentifier.get(ArticlesList, ArticlesListParams(listType = ArticlesListType.ALL)), true),
    FavoriteArticles( ScreenIdentifier.get(FavoriteArticlesList, ArticlesListParams(listType = ArticlesListType.FAVORITES)), true),
}